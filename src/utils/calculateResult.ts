...
/**
 * Question weights - Balanced distribution for comprehensive diagnosis
 * All questions contribute meaningfully to the result
 */
const QUESTION_WEIGHTS = [0.18, 0.12, 0.12, 0.18, 0.18, 0.22];
...
export async function calculateOnsenType(answers: QuizAnswers): Promise<DiagnosisResult> {
  ...
  // Penalize overrepresented types by 8% of their frequency
  const frequencies = await getFrequencyData();
  ALL_TYPES.forEach(type => {
    totalScores[type] = totalScores[type] * (1 - 0.08 * frequencies[type]);
  });
  
  // Step 5: Normalize to percentages (0-100)
  ...
  if (primary.score >= 70) confidence = 'high';
