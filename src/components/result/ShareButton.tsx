...
import { socialShareUtils } from '@/utils/social-share';
...
export const ShareButton = ({ title }: ShareButtonProps) => {
  ...
  const handleInstagramShare = async () => {
    ...
    try {
      await socialShareUtils.copyToClipboard(url);
      toast({
        title: SOCIAL_SHARE_TEXT.INSTAGRAM_COPY_SUCCESS,
        description: SOCIAL_SHARE_TEXT.INSTAGRAM_COPY_DESCRIPTION,
      });
    } catch (error) {
      toast({
        title: SOCIAL_SHARE_TEXT.COPY_ERROR,
        description: SOCIAL_SHARE_TEXT.COPY_ERROR_DESCRIPTION,
        variant: "destructive",
      });
    }
  };
        ...
        {showShareMenu && (
          <div className="absolute top-16 right-0 bg-background/95 backdrop-blur-lg border border-border rounded-lg shadow-xl p-3 space-y-2 animate-fade-in will-change-[opacity,transform] min-w-[160px] max-w-[90vw]">
            <Button
              ...
              variant="outline"
              size="sm"
              className="w-full justify-start text-muted-foreground hover:text-foreground hover:bg-accent/10 hover:border-primary/50"
            ...
            <Button
              onClick={() => socialShareUtils.shareToFacebook()}
              variant="outline"
              size="sm"
            ...
            <Button
              onClick={() => socialShareUtils.shareToWhatsApp(title)}
              variant="outline"
              size="sm"
