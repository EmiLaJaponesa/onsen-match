  ...
  return (
    ...
    >
      {options.map((option) => (
        <div 
          key={option.id} 
          className={`group relative flex items-center space-x-4 p-5 rounded-xl border-2 transition-all duration-200 cursor-pointer touch-target
        ...
        >
          <RadioGroupItem 
            value={option.id}
            id={option.id}
            className="transition-smooth radio-button flex-shrink-0"
          />
          <Label 
            htmlFor={option.id}
            className="flex-1 cursor-pointer text-base md:text-lg leading-relaxed radio-label"
          >
            {option.text}
          </Label>
        </div>
